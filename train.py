...
def parse_options(root_path, is_train=True):
    ...
    parser.add_argument('--local_rank', type=int, default=0)  # the rank in a machine.
    ...
    set_random_seed(seed + opt['rank'])  # random seed in each GPU should be different
...
def init_loggers(opt):
    ...
    if opt['debug']:
        log_file = None
    logger = get_root_logger(logger_name='basicsr', log_level=logging.INFO, log_file=log_file, flag=False)
    logger.info(dict2str(opt))
    tb_logger = None
    if opt['logger'].get('use_tb_logger') and (not opt['debug']):
...
def train_pipeline(root_path):
    ...
    if resume_state is None and (not opt['debug']):
        make_exp_dirs(opt)
...
if __name__ == '__main__':
    ...
    root_path = osp.abspath(osp.join(__file__, osp.pardir, osp.pardir, 'VOTES'))
